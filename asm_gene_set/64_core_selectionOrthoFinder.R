...
tree <- ape::read.tree("/vast/eande106/projects/Lance/THESIS_WORK/assemblies/orthology/elegans/Ce_phy_file_LD_0.9.phy.contree")

# Patristic distance between all strains - how evolutionarily far apart are the strains? pairwise
D <- ape::cophenetic.phylo(tree)

strainsIhave <- readr::read_tsv("/vast/eande106/projects/Lance/THESIS_WORK/assemblies/assembly-nf/all_assemblies_sheet/140_Ce_WSs.tsv", col_names = "strain") 

tree$tip.label <- stringr::str_trim(tree$tip.label)

have_vec <- strainsIhave$strain
tips     <- tree$tip.label
...
present_core_set <- strainsIhave %>% dplyr::filter(strain %in% core_set)

write.table(present_core_set, "/vast/eande106/projects/Lance/THESIS_WORK/assemblies/orthology/elegans/core64_FINAL.tsv", quote = F, col.names = F, row.names = F)
