...
for (i in 1:length(strains)) {
  ...
  sg_ogs <- all_relations %>% dplyr::filter(Orthogroup %in% hdr_ogs) %>% dplyr::filter(Orthogroup %in% sc_ogs) %>% dplyr::distinct(Orthogroup) %>% dplyr::pull()
  ...
  sg_ogs_nonHDR <- all_relations %>% dplyr::filter(!Orthogroup %in% hdr_ogs) %>% dplyr::filter(Orthogroup %in% sc_ogs) %>% dplyr::distinct(Orthogroup) %>% dplyr::pull()
...
plot_df_norm <- cnv_pav_results_df %>%
